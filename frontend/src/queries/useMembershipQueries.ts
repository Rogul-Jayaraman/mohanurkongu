...
import { getProfile as getAccountProfile } from '../api/auth.api';
...
export function useMyCapabilitiesQuery() {
  return useQuery({
    queryKey: queryKeys.membership.caps(),
...
export function useBillingOverviewQuery() {
  return useQuery({
    queryKey: queryKeys.membership.billing(),
