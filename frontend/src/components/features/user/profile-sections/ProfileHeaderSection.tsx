...
const ProfileHeaderSection: React.FC<ProfileHeaderSectionProps> = ({ profile, isLoading, isTamil, isOwner }) => {
  ...
  const ageDisplay = profile?.dob ? `${calculateAge(profile.dob)} ${tCommon('yrs')}` : '';
  const genderLabel = profile?.gender ? getEnumLabel(profile.gender, GENDER_OPTIONS) : '';
    ...
    <div className="grid grid-cols-1 md:grid-cols-12 gap-6 sm:gap-8">
      <div className="md:col-span-8">
        <SectionCard3D>
          ...
          <div className="space-y-0">
            <DetailRow label={t('profile_new:full_name')} value={name} />
            <DetailRow label={tCommon('profile.reg_no')} value={profile?.regNo} />
            <DetailRow label={t('profile_new:dob')} value={profile?.dob ? formatDate(profile.dob) : ''} />
            <DetailRow label={tCommon('profile.age')} value={ageDisplay} />
            <DetailRow label={t('profile_new:gender')} value={genderLabel} />
            <DetailRow label={t('profile_new:current_location')} value={currentLocation} />
          </div>
        </SectionCard3D>
      </div>
      <div className="md:col-span-4">
          ...
          <div className="w-full max-w-56 sm:max-w-none aspect-square rounded-xl overflow-hidden border-4 border-ivory bg-ivory">
            {profilePhotoUrl ? (
              ...
              <div className="w-full h-full bg-ivory flex items-center justify-center">
                <span className="text-rosewood/30 text-4xl font-serif font-bold">{name?.charAt(0)}</span>
              </div>
            )}
          </div>
        </div>
