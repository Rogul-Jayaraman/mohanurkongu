...
import { RESIDENCE_OPTIONS } from '@/constants/index';
...
const AssetsSection: React.FC<AssetsSectionProps> = ({ profile, isLoading }) => {
  ...
  return (
    <SectionCard3D isLoading={isLoading}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-0">
        <DetailRow label={t('profile_new:residence')} value={profile?.residence ? getEnumLabel(profile.residence, RESIDENCE_OPTIONS) : ''} isLoading={isLoading} />
        <DetailRow label={t('profile_new:vehicle')} value={profile?.vehicle || '-'} isLoading={isLoading} />
        <DetailRow label={t('profile_new:other_assets')} value={otherAssets || '-'} isLoading={isLoading} />
        <DetailRow label={t('profile_new:land')} value={land || '-'} isLoading={isLoading} />
      </div>
    </SectionCard3D>
  );
};
