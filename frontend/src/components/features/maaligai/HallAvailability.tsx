...
import { usePublicCalendar } from '@/queries/useMandapamQueries';
import {
    getTamilDateInfo,
    CALENDAR_THEME,
    type CalendarStatus
...
export const SharedCalendar: React.FC<SharedCalendarProps> = ({
    ...
    bookedDays, blockedDays = [], partiallyBookedDays = [], isAdmin = false, language, legend, daysOfWeek
}) => {
    ...
    return (
        <div className="ha-calendar bg-background-light rounded-xl md:rounded-2xl p-1 sm:p-3 md:p-5 border border-gold-accent/40 shadow-lg w-full">
            ...
            >
                ...
                <div className={`flex flex-wrap items-center justify-center gap-x-4 gap-y-1.5 mt-3 md:mt-4 text-[9px] md:text-[11px] font-bold text-rosewood/50`}>
                    <div className="flex items-center gap-1.5">
                        <span className={`w-2 h-2 rounded-sm ${CALENDAR_THEME.selected.legend}`}></span>
                        <span>{legend.selected}</span>
                    </div>
                    {legend.booked && (
                        <div className="flex items-center gap-1.5">
                            <span className={`w-2 h-2 rounded-sm ${CALENDAR_THEME.booked.legend}`}></span>
                            <span>{legend.booked}</span>
                        </div>
                    )}
                    {legend.partiallyBooked && (
                        <div className="flex items-center gap-1.5">
                            <span className={`w-2 h-2 rounded-sm ${CALENDAR_THEME.partiallyBooked.legend}`}></span>
                            <span>{legend.partiallyBooked}</span>
                        </div>
                    )}
                    <div className="flex items-center gap-1.5">
                        <span className={`w-2 h-2 rounded-sm ${CALENDAR_THEME.available.legend}`}></span>
                        <span>{legend.available}</span>
                    </div>
                    {legend.blocked && (
                        <div className="flex items-center gap-1.5">
                            <span className={`w-2 h-2 rounded-sm ${CALENDAR_THEME.blocked.legend}`}></span>
                            <span>{legend.blocked}</span>
                        </div>
                    )}
                    {legend.today && (
                        <div className="flex items-center gap-1.5">
                            <span className={`w-2 h-2 rounded-sm ${CALENDAR_THEME.today.legend}`}></span>
                            <span>{legend.today}</span>
                        </div>
                    )}
                    {legend.disabled && (
                        <div className="flex items-center gap-1.5">
                            <span className={`w-2 h-2 rounded-sm ${CALENDAR_THEME.disabled.legend}`}></span>
                            <span>{legend.disabled}</span>
                        </div>
                    )}
                    {selectedDay && legend.clear && (
                        <button
                            onClick={() => onSelectDay(null)}
                            className="px-2 py-0.5 rounded text-[9px] font-bold text-rosewood/60 border border-rosewood/30 hover:bg-rosewood hover:text-white hover:border-rosewood transition-all active:scale-95"
                        >
                            {legend.clear}
                        </button>
                    )}
            ...
            <div className="grid grid-cols-7 gap-px mb-px">
                {daysOfWeek.map((d: string) => (
                    <span key={d} className={`${ls('text-[8px] sm:text-[10px] md:text-xs tracking-wider uppercase', 'text-[8px] sm:text-[10px] md:text-xs tracking-normal')} py-2 md:py-2.5 font-bold text-center text-rosewood/60 bg-ivory-tint rounded-sm`}>{d}</span>
                ))}
            </div>

            <div className="grid grid-cols-7 gap-px bg-sage-green/20 rounded-lg overflow-hidden">
                ...
                {days.map((day, idx) => {
                    ...
                    const isDisabled = isPast || (!isAdmin && (isBooked || isBlocked));
                    ...
                    if (isAdmin) {
                        ...
                        if (isBlocked || isBooked) status = 'blocked';
                    }
                    if (status === 'available') {
                    ...
                    return (
                        <motion.button
                            ...
                            disabled={isDisabled}
                            className={`aspect-square relative flex flex-col items-center justify-center transition-all rounded-sm ${theme.cell} ${(!isDisabled && status !== 'booked' && status !== 'blocked') ? 'hover:scale-[1.08] active:scale-95 cursor-pointer' : 'cursor-default'}`}
                        >
                            {showTamilLabel && (
                             ...
                             <span className={`font-heading font-bold ${ls('text-sm sm:text-xl md:text-3xl', 'text-sm sm:text-xl md:text-3xl')} ${theme.textMain}`}>
                                  {day}
                             </span>
...
export const AvailabilityCalendar: React.FC<AvailabilityCalendarProps> = () => {
    ...
    const { data: calendarData, isLoading } = usePublicCalendar(from, to);
    const entries = calendarData?.entries ?? [];

    const { bookedDays, blockedDays, partiallyBookedDays } = useMemo(() => {
        ...
        return { bookedDays: bkd, blockedDays: blk, partiallyBookedDays: pb };
    }, [entries, currentDate]);
    ...
    if (isLoading) {
        return (
            <section id="ha-calendar" className="px-4 md:px-6 lg:px-20 section-spacing bg-white">
                <div className="max-w-3xl mx-auto flex flex-col items-center justify-center min-h-[400px]">
                    <div className="flex flex-col items-center gap-4">
                        <div className="w-10 h-10 border-4 border-rosewood/30 border-t-rosewood rounded-full animate-spin" />
                        <p className="text-rosewood/60 font-bold text-xs uppercase tracking-widest">
                    ...
                    <div>
                        <SharedCalendar
                            ...
                            legend={{
                                ...
                                partiallyBooked: content.availabilityLegend.partiallyBooked,
                                blocked: content.availabilityLegend.blocked,
