    ...
    if (addons.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center py-16 bg-ivory border-2 border-dashed border-gold/20 rounded-2xl">
                ...
                <p className="text-rosewood/60 font-bold">{t('adminMandapam.addons.noAddonsFound')}</p>
                <p className="text-rosewood/40 text-sm mt-1">{t('adminMandapam.addons.noAddonsDesc')}</p>
            </div>
        );
    }
