...
interface Props {
  title: string; subtitle?: string; data: Record<string, any>[]; keys: { key: string; color: string; name: string }[];
        ...
        <div className="p-4"><div style={{ height }}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data} margin={{ top: 8, right: 8, left: -8, bottom: 0 }}>
              ...
              {keys.map(k => <Bar key={k.key} dataKey={k.key} stackId={stacked ? 's' : undefined} fill={k.color} name={k.name} radius={[2, 2, 0, 0]} barSize={stacked ? 24 : 12} />)}
            </BarChart>
          </ResponsiveContainer>
        </div></div>
      )}
