...
interface Series {
  key: string; type: 'bar' | 'line'; color: string; name: string; yAxisId?: string;
}

interface Props {
  title: string; subtitle?: string; data: Record<string, any>[]; series: Series[];
...
export const TrendComposedChart: React.FC<Props> = React.memo(({ title, subtitle, data, series, xKey, loading, height = 300 }) => {
  ...
  return (
    <div className="bg-white border border-gold/20 rounded-xl shadow-sm">
      ...
      {data.length === 0 ? (
        <div className="p-6"><EmptyState message="No trend data yet" icon={TrendingUp} variant="dashed" /></div>
      ) : (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={data} margin={{ top: 8, right: 8, left: -8, bottom: 0 }}>
              ...
              <Tooltip content={({ active, payload, label }: any) => {
                ...
                return <div className="bg-white/95 border border-gold/20 rounded-xl shadow-lg px-4 py-3 text-xs"><p className="font-serif font-bold text-rosewood mb-1">{label}</p>{payload.map((p: any, i: number) => <p key={i} className="text-slate-600" style={{ color: p.color }}>{p.name}: <span className="font-bold">{typeof p.value === 'number' ? p.value.toLocaleString() : p.value}</span></p>)}</div>;
              }} />
              <Legend iconType="circle" iconSize={8} wrapperStyle={{ fontSize: 11, paddingTop: 8 }} />
              {series.map(s => {
                if (s.type === 'bar') return <Bar key={s.key} yAxisId={s.yAxisId || 'left'} dataKey={s.key} fill={s.color} name={s.name} radius={[4, 4, 0, 0]} barSize={12} />;
                return <Line key={s.key} yAxisId={s.yAxisId || 'left'} type="monotone" dataKey={s.key} stroke={s.color} name={s.name} strokeWidth={2} dot={{ r: 3, fill: s.color }} activeDot={{ r: 5 }} />;
              })}
            </ComposedChart>
          </ResponsiveContainer>
        </div></div>
