...
export const RenewalForecastChart: React.FC<Props> = React.memo(({ title, subtitle, data, loading }) => {
  if (loading) {
    return (
      <div className="bg-white border border-gold/20 rounded-xl shadow-sm animate-pulse">
        <div className="p-5 border-b border-gold/10">
          <div className="h-4 w-48 bg-rosewood/10 rounded" />
          {subtitle && <div className="h-3 w-32 bg-rosewood/5 rounded mt-1.5" />}
        </div>
        ...
        <div className="p-4"><div style={{ height: 280 }}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data} margin={{ top: 8, right: 8, left: -8, bottom: 0 }}>
              ...
              {KEYS.map(k => (
                <Bar key={k.key} dataKey={k.key} stackId="s" fill={k.color} name={k.name} radius={[2, 2, 0, 0]} barSize={32} />
              ))}
            </BarChart>
          </ResponsiveContainer>
        </div></div>
...
RenewalForecastChart.displayName = 'RenewalForecastChart';
