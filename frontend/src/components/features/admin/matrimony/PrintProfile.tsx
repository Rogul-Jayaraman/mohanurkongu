...
import {
    ...
    HEIGHT_OPTIONS, KULAM_OPTIONS, RESIDENCE_OPTIONS,
    JOB_SECTOR_OPTIONS, PROFILE_FOR_OPTIONS
...
const DataRow: React.FC<{ label: string; value: string }> = ({ label, value }) => (
    <div className="flex items-baseline py-1 border-b border-gray-200 last:border-b-0">
        <span className="text-gray-500 text-[10px] font-semibold w-[130px] shrink-0">{label}</span>
...
const SectionTitle: React.FC<{ title: string }> = ({ title }) => (
    <div className="mb-3 pb-1.5 border-b border-gray-300">
        <h2 className="text-gray-700 text-[12px] font-bold">{title}</h2>
    </div>
);
...
const JDataRow: React.FC<{ label: string; value: string }> = ({ label, value }) => (
    <div className="flex py-1 border-b border-gray-200 last:border-0 items-baseline">
        <span className="w-[90px] text-[8px] font-semibold text-gray-500 whitespace-nowrap">{label}</span>
...
const PrintBasicDetail: React.FC<{ isTamil: boolean; age: string; maritalStatus: string; height: string; weight: string; diet: string; complexion: string; bloodGroup: string; currentLocation: string; gender: string; dob: string }> = ({ isTamil, age, maritalStatus, height, weight, diet, complexion, bloodGroup, currentLocation, gender, dob }) => (
    <div className="col-span-12 p-4 border border-gray-200">
        ...
        <div className="grid grid-cols-2 gap-x-6">
            ...
            <div>
                ...
                <DataRow label={isTamil ? 'உணவு' : 'Diet'} value={diet} />
                <DataRow label={isTamil ? 'நிறம்' : 'Complexion'} value={complexion} />
...
const PrintPersonalInfo: React.FC<{
    ...
    occupation: string; companyName: string; jobLocation: string; jobSector: string; residence: string;
    motherName: string; motherJob: string; sisters: number;
}> = ({ isTamil, profileFor, education, monthlyIncome, nativeLocation, birthPlace, fatherName, fatherJob, brothers, occupation, companyName, jobLocation, jobSector, residence, motherName, motherJob, sisters }) => (
    <div className="col-span-12 p-4 border border-gray-200">
        <SectionTitle title={isTamil ? 'தனிப்பட்ட தகவல்கள்' : 'Personal Information'} />
        <div className="grid grid-cols-2 gap-x-6">
            <div>
                <DataRow label={isTamil ? 'விண்ணப்பதாரர்' : 'Profile For'} value={profileFor} />
                ...
                <DataRow label={isTamil ? 'பிறந்த இடம்' : 'Birth Place'} value={birthPlace} />
            ...
            <div>
                ...
                <DataRow label={isTamil ? 'நிறுவனம்' : 'Company Name'} value={companyName} />
                <DataRow label={isTamil ? 'பணிபுரியும் இடம்' : 'Job Location'} value={jobLocation} />
                <DataRow label={isTamil ? 'பணித் துறை' : 'Job Sector'} value={jobSector} />
...
const PrintJathagamSection: React.FC<{ profile: any; isTamil: boolean; star: string; rasi: string; lagna: string; birthTime: string; birthPlace: string }> = ({ profile, isTamil, star, rasi, lagna, birthTime, birthPlace }) => {
    const hasCharts = profile?.horoscope && !profile?.horoscopeLocked && (
        profile.horoscope.rasi ||
        profile.horoscope.navamsa ||
        (profile.horoscope.mode === 'GENERATED' && profile.horoscope.horoscopeJson)
    );

    const renderChart = (type: 'rasi' | 'navamsa') => {
        ...
        if (profile.horoscope?.mode === 'GENERATED' && hJson) {
            ...
            return (
                <div className="flex flex-col items-center">
                    <span className="text-[9px] font-bold text-gray-500 mb-2">{label}</span>
                    <div className="w-[260px] h-[260px] bg-white border border-gray-200 print-chart-box">
                        <SouthIndianChart lagnaSignIndex={isRasi ? parsed.lagna.signIndex : parsed.lagnaNavamsa.signIndex} planets={isRasi ? parsed.planets : d9Planets} rotateHouses={isRasi} />
                    </div>
                </div>
            );
        }
    ...
    return (
        <div className="col-span-12 p-4 border border-gray-200">
            ...
            <div className="grid grid-cols-2 gap-2">
                ...
                <DataRow label={isTamil ? 'பிறந்த நேரம்' : 'Birth Time'} value={birthTime} />
                <DataRow label={isTamil ? 'பிறந்த இடம்' : 'Birth Place'} value={birthPlace} />
            </div>
            {hasCharts && (
...
const PrintFooter: React.FC<{ regNo: string; isTamil: boolean }> = ({ regNo, isTamil }) => (
    <footer className="mt-4 pt-3 border-t border-gray-300">
        <div className="flex justify-between items-center text-gray-500">
            <div>
                <p className="text-[8px] font-bold">{isTamil ? 'மோகனூர் கொங்கு மணமாலை' : 'MOHANUR KONGU MANAMAALAI'}</p>
                <p className="text-[7px] mt-0.5">{isTamil ? 'திருமண நோக்கத்திற்காக மட்டுமே' : 'For private matrimonial purposes only'}</p>
            </div>
            <div className="text-right">
                <p className="text-[8px] font-semibold">Printed: {new Date().toLocaleDateString(isTamil ? 'ta-IN' : 'en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}</p>
...
const PrintProfile: React.FC<{ profile: any }> = ({ profile }) => {
    ...
    const jobSector = profile.jobSector ? getEnumLabel(profile.jobSector, JOB_SECTOR_OPTIONS) : '-';
            ...
            <style>{`
                ...
                @media print {
                    ...
                    * { -webkit-print-color-adjust: exact; }
                    .print-chart-box { display: flex !important; align-items: center !important; justify-content: center !important; overflow: visible !important; }
                    ...
                    .skip-link, nav, header:not(#premium-print-layout header),
                    footer:not(#premium-print-layout footer), button:not(#premium-print-layout button),
                    .action-buttons, .back-btn, .kolam-watermark, .no-print {
...
<h1 className="font-serif text-2xl text-gray-900 leading-tight font-bold">{name}</h1>
                ...
                <div className="grid grid-cols-2 gap-3 mb-3">
                    <div className="border border-gray-200 p-3">
                        <h2 className="text-[10px] font-bold text-gray-700 mb-2 pb-1.5 border-b border-gray-200">
                            {isTamil ? 'தனிப்பட்ட விவரங்கள்' : 'Personal Details'}
                        </h2>
                        <JDataRow label={isTamil ? 'பிறந்த தேதி' : 'DOB'} value={dob} />
                        <JDataRow label={isTamil ? 'விண்ணப்பதாரர்' : 'Profile For'} value={profileFor} />
                ...
                {!profile?.horoscope?.locked && !profile?.horoscopeLocked && (
                <div className="border border-gray-200 p-3 mb-3">
                    <h2 className="text-[10px] font-bold text-gray-700 mb-4 text-center pb-2 border-b border-gray-200">
                            ...
                            {(() => {
                                ...
                                if (profile.horoscope?.mode === 'GENERATED' && hJson) {
                                    ...
                                    return (
                                        <>
                                            <div className="flex flex-col items-center">
                                                <span className="text-[8px] font-bold text-gray-500 mb-2">{isTamil ? 'ராசி' : 'RASI'}</span>
                                                <div className="w-[260px] h-[260px] bg-white border border-gray-200 print-chart-box">
                                                    <SouthIndianChart lagnaSignIndex={parsed.lagna.signIndex} planets={parsed.planets} rotateHouses={true} />
                                                </div>
                                            </div>
                                            <div className="flex flex-col items-center">
                                                <span className="text-[8px] font-bold text-gray-500 mb-2">{isTamil ? 'நவாம்சம்' : 'NAVAMSA'}</span>
                                                <div className="w-[260px] h-[260px] bg-white border border-gray-200 print-chart-box">
                                    ...
                                    <>
                                        {profile.horoscope.rasi?.url && (
                                            <div className="flex flex-col items-center">
                                                <span className="text-[8px] font-bold text-gray-500 mb-2">{isTamil ? 'ராசி' : 'RASI'}</span>
                                                <img
                                                    src={getImageUrl(profile.horoscope.rasi.url) || ''}
                                                    alt="Rasi"
                                                    className="w-[260px] h-[260px] object-contain border border-gray-200 bg-white"
                                                />
                                            </div>
                                        ...
                                        {profile.horoscope.navamsa?.url && (
                                            <div className="flex flex-col items-center">
                                                <span className="text-[8px] font-bold text-gray-500 mb-2">{isTamil ? 'நவாம்சம்' : 'NAVAMSA'}</span>
                                                <img
                                                    src={getImageUrl(profile.horoscope.navamsa.url) || ''}
                                                    alt="Navamsa"
                                                    className="w-[260px] h-[260px] object-contain border border-gray-200 bg-white"
                                                />
                                            </div>
                ...
                <div className="pt-2 border-t border-gray-200">
                    <div className="flex justify-between items-center text-[8px] text-gray-500">
                        <div>
                            <p className="font-bold">{isTamil ? 'மோகனூர் கொங்கு மணமாலை' : 'MOHANUR KONGU MANAMAALAI'}</p>
                            <p className="mt-0.5">{isTamil ? 'திருமண நோக்கத்திற்காக மட்டுமே' : 'For matrimonial purposes only'}</p>
                        </div>
                        <div className="text-right">
                            <p className="font-semibold">{isTamil ? 'அச்சிட்ட தேதி' : 'Printed'}: {new Date().toLocaleDateString(isTamil ? 'ta-IN' : 'en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}</p>
            ...
            <style>{`
                ...
                @media print {
                    ...
                    .hidden-jathagam-container { display: block !important; width: 100% !important; margin: 0 !important; padding: 0 !important; height: auto !important; min-height: 0 !important; }
                    .hidden-jathagam-container > div { padding: 0 !important; }
                    * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
                    .print-chart-box { display: flex !important; align-items: center !important; justify-content: center !important; overflow: visible !important; }
                    .print-chart-box > div { width: 100% !important; height: 100% !important; display: flex !important; flex-direction: column !important; }
                    .print-chart-box svg { width: 100% !important; height: 100% !important; display: block !important; }

                    .skip-link, nav, header:not(.hidden-jathagam-container header),
                    footer:not(.hidden-jathagam-container footer), button:not(.hidden-jathagam-container button),
                    .action-buttons, .back-btn, .kolam-watermark, .no-print {
                        display: none !important;
                    }
                }
