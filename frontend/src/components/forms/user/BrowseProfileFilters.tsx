...
import { Lock, Search, RotateCcw, X, Star, MapPin, Users, User, GraduationCap, Heart } from 'lucide-react';

const ICON_GRADIENTS: Record<string, string> = {
    astrology: 'bg-linear-to-br from-rosewood/80 via-dark-rosewood/95 to-rosewood/80 text-white',
    location: 'bg-linear-to-br from-ivory to-gold/40 text-rosewood',
    community: 'bg-linear-to-br from-rosewood/80 via-dark-rosewood/95 to-rosewood/80 text-white',
    physical: 'bg-linear-to-br from-ivory to-gold/40 text-rosewood',
    personal: 'bg-linear-to-br from-rosewood/80 via-dark-rosewood/95 to-rosewood/80 text-white',
    education: 'bg-linear-to-br from-ivory to-gold/40 text-rosewood',
};

const SECTION_ICONS: Record<string, React.ReactNode> = {
    astrology: <Star size={16} />,
    location: <MapPin size={16} />,
    community: <Users size={16} />,
    physical: <User size={16} />,
    personal: <Heart size={16} />,
            ...
            {activeKeys.map(key => {
                ...
                return (
                    <motion.div
                        ...
                        className="flex items-center gap-2 pl-3 pr-2 py-1.5 bg-white/40 backdrop-blur-md border border-gold/20 rounded-xl shadow-sm hover:border-gold transition-all group"
                    >
                        <div className="flex flex-col leading-tight">
                            <span className="text-[7px] font-black text-rosewood/40 uppercase tracking-widest">{info.category}</span>
                            <span className="text-[11px] font-bold text-rosewood">{info[lang === 'ta' ? 'ta' : 'en']}</span>
                        </div>
                        <button onClick={() => onClear(key)} className="size-6 flex items-center justify-center rounded-lg bg-rosewood/5 hover:bg-rosewood text-rosewood/40 hover:text-white transition-all duration-300">
                            <X size={12} />
                        </button>
                    </motion.div>
                );
...
const LockedOverlay: React.FC<{ message: string; messageTa: string; gradientKey: string }> = ({ message, messageTa, gradientKey }) => {
    const { i18n } = useTranslation();
    const isTamil = i18n.language === 'ta';
    return (
        <div className="absolute inset-0 z-20 flex items-center justify-center bg-ivory/80 backdrop-blur-[2px] rounded-xl">
            <motion.div
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
                className="flex flex-col items-center gap-3 px-8 py-6 bg-white rounded-2xl shadow-lg border border-gold/20 mx-4"
            >
                <div className={`size-14 rounded-full flex items-center justify-center border ${ICON_GRADIENTS[gradientKey]} border-gold/10 shadow-sm`}>
                    <Lock size={22} />
                </div>
                <p className="text-xs font-semibold text-rosewood/60 text-center max-w-[220px] leading-relaxed">
...
const SectionCard: React.FC<SectionCardProps> = ({
    iconKey, title, children, columns = 1, spanClassName = '',
    locked = false, lockedMessage = '', lockedMessageTa = ''
}) => {
    return (
        <motion.div
            ...
            className={`bg-ivory rounded-xl shadow-sm border border-gold/20 px-4 sm:px-6 md:px-8 py-4 sm:py-5 md:py-6 h-full relative overflow-hidden hover:shadow-md transition-shadow duration-500 ${spanClassName}`}
        >
            <div className="flex items-center gap-3 mb-4 border-b border-gold/10 pb-2">
                <div className={`p-1.5 sm:p-2.5 rounded-xl shadow-sm shrink-0 ${ICON_GRADIENTS[iconKey]}`}>
                    {SECTION_ICONS[iconKey]}
                </div>
                <h2 className="font-serif text-rosewood font-black flex-1 text-lg sm:text-xl">{title}</h2>
            </div>
            <div className={`gap-x-6 gap-y-4 ${
                columns === 1 ? 'flex flex-col space-y-3' :
                columns === 2 ? 'grid grid-cols-1 md:grid-cols-2' :
                'grid grid-cols-1 md:grid-cols-3'
            }`}>
                {children}
                {locked && <LockedOverlay message={lockedMessage} messageTa={lockedMessageTa} gradientKey={iconKey} />}
            </div>
        </motion.div>
    );
};
...
const FilterDropdown = ({ label, options, value, onChange, placeholder, bilingual }: {
    ...
    <div className="space-y-1.5">
        <label className="block text-[11px] sm:text-xs font-bold text-rosewood tracking-tight ml-1">{label}</label>
        <Dropdown
            options={options}
            value={value}
            onChange={onChange}
                    ...
                    >
                        ...
                        <div className="shrink-0 relative z-100 px-6 lg:px-12 py-5 bg-white/80 backdrop-blur-2xl border-b border-gold/10 flex items-center justify-between shadow-sm">
                            <div className="flex items-center gap-5">
                                <div className="size-11 rounded-xl bg-linear-to-br from-rosewood/80 via-dark-rosewood/95 to-rosewood/80 flex items-center justify-center shadow-lg shadow-rosewood/20 relative overflow-hidden">
                        ...
                        <div className="flex-1 overflow-y-auto filter-scroll-container custom-scrollbar relative z-10 bg-slate-50/30">
                            <div className="max-w-7xl mx-auto px-6 lg:px-12 py-12 grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
                                <SectionCard
                                    iconKey="location"
                                    title={t('browse:section_location')}
                                    columns={1}
                                >
                                        ...
                                        {residenceLocked && (
                                            <div className="absolute inset-0 z-10 flex items-center justify-center bg-white/60 backdrop-blur-[1px] rounded-lg">
                                                <div className="flex flex-col items-center gap-2">
                                                    <Lock size={14} className="text-rosewood/30" />
                                ...
                                <SectionCard
                                    iconKey="community"
                                    title={t('browse:section_community')}
                                    columns={2}
                                >
                                    <FilterDropdown label={t('browse:kulam')} options={kulamDropdownOpts} value={localFilters.kulam || ''} onChange={v => handleChange('kulam', v)} />
                                    <Input label={t('browse:kuladeivam')} icon="temple_hindu" name="kuladeivam" placeholder={t('browse:type_name_placeholder', 'Type name...')} value={localFilters.kuladeivam || ''} onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleChange('kuladeivam', e.target.value)} className="" />
                                ...
                                <SectionCard
                                    iconKey="physical"
                                    title={t('browse:section_physical')}
                                    columns={1}
                                >
                                    <RangeSlider min={21} max={40} value={[Number(localFilters.minAge) || 21, Number(localFilters.maxAge) || 40]} onChange={(val: number[]) => { handleChange('minAge', val[0]); handleChange('maxAge', val[1]); }} label={t('browse:age_range')} />
                                ...
                                <SectionCard
                                    iconKey="personal"
                                    title={t('browse:section_personal')}
                                    columns={1}
                                >
                                ...
                                <SectionCard
                                    iconKey="education"
                                    title={t('browse:section_education')}
                                    columns={2}
                                ...
                                >
                                    ...
                                    <MultiSelect options={jobSectorOpts} values={localFilters.jobSector || []} onChange={(vals: string[]) => handleChange('jobSector', vals)} label={t('browse:job_sector')} placeholder={t('browse:job_sector')} />
                                    <div className="md:col-span-2">
                                ...
                                <SectionCard
                                    iconKey="astrology"
                                    title={t('browse:section_astrology')}
                                    columns={1}
                                    spanClassName="md:col-span-2"
