...
interface DualScriptFieldProps {
    value: string;
...
export const DualScriptField: React.FC<DualScriptFieldProps> = ({
    value,
    onChange,
    placeholder,
    label,
    ...
    const {
        ...
        insertKey,
        handleBackspace,
    } = useDualScript({ text: value, targetLanguage: 'ta' });
        ...
        <div className={`w-full space-y-2 ${className}`}>
            ...
            <div className="relative group">
                <input
                    ...
                    disabled={disabled}
                    className="w-full h-12 px-4 pr-12 rounded-xl border border-input-border bg-input-bg text-sm text-input-text placeholder:text-input-placeholder outline-none focus:border-input-focus focus:ring-4 focus:ring-input-ring transition-all disabled:opacity-70 disabled:cursor-not-allowed"
                />
                {isPending && (
