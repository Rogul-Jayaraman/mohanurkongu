...
interface DualScriptTextareaProps {
    label: string;
    valueEn: string;
    valueTa: string;
...
export const DualScriptTextarea: React.FC<DualScriptTextareaProps> = ({
    label,
    valueEn,
    valueTa,
    onChangeEn,
    ...
    const [activeScript, setActiveScript] = useState<'en' | 'ta'>('en');
        ...
        <div className="w-full space-y-3">
            ...
            <div className="relative group">
                <textarea
                    ...
                    className={`w-full min-h-[160px] p-4 pr-12 rounded-xl border transition-all text-sm placeholder:text-input-placeholder text-input-text resize-none outline-none ${
                        error
                            ? 'border-red-200 focus:border-red-400 focus:ring-4 focus:ring-red-400/5'
                            : 'border-input-border focus:border-input-focus focus:ring-4 focus:ring-input-ring shadow-sm bg-input-bg hover:border-input-border-hover'
                    }`}
                />
                {activeScript === 'ta' && (
                    <button
                        ...
                        className={`absolute right-4 bottom-4 size-8 flex items-center justify-center rounded-lg transition-all z-10 ${
                            isKeyboardOpen
                                ? 'bg-rosewood text-white shadow-lg'
                                : 'bg-gold/5 text-rosewood hover:bg-gold/10 border border-gold/10'
                        }`}
                    >
